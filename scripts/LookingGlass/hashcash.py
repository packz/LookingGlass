...
def check(stamp, resource=None, bits=None,
    ...
    """
    if stamp.startswith('0:'):          # Version 0 - oh hail no
        return False
        ...
        if resource is not None and resource != res:
            ...
            # FIXME: this is crap - we should be using DateTime or epoch
            good_until = strftime("%y%m%d%H%M%S", localtime(time()-check_expiration))
            if date < good_until:
                return False
        elif callable(ds_callback) and ds_callback(stamp):
            return False
        # we made it through the major hurdles - let's actully do this thing.
