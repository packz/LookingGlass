...
class Updater(object):
    ...
    def __repr__(self):
        CurrentMaj, CurrentMin = TTS.LOOKINGGLASS_VERSION
    ...
    def GetMostRecent(self, asString=False):
        Major = self.ReqMajor
        Minor = self.ReqMinor
        GV = self.GetVersion(Major, Minor)
        if not asString:
            return GV
        elif GV is None:
            # FIXME: this is a lie
            return 'LookingGlass V%02d.%02d' % (TTS.LOOKINGGLASS_VERSION[0], TTS.LOOKINGGLASS_VERSION[1])
        elif GV.has_key('deb'):
            return 'LookingGlass V%02d.%02d' % (int(GV['deb']['Major']), int(GV['deb']['Minor']))   
        elif GV.has_key('tar.bz2'):
            return 'LookingGlass V%02d.%02d' % (int(GV['tar.bz2']['Major']), int(GV['tar.bz2']['Minor']))
    ...
    def Scan(self, Server=None):
        ...
        ret = []
        FNRE = re.compile(
            'LookingGlass_(?P<Major>[0-9]+)\.(?P<Minor>[0-9]+)_(rpi|all).(?P<Type>sum|tar.bz2|deb)$')
        if Server:
            SO, SE = utils.popen_wrapper(['/usr/bin/rsync', '%s/Upgrade' % Server],
                                         sudo=False)
            ...
            for Line in SO.split('\n'):
                ...
                if Z:
                    ...
                    Q['Exact'] = '%s/Upgrade/%s' % (Server,
                                                    Filename)
                    ret.append(Q)
                    if ((int(Z.group('Major')) > self.ReqMajor) or # major rev
                        ((int(Z.group('Major')) == self.ReqMajor) and # same major, bigger minor
                         (int(Z.group('Minor')) > self.ReqMinor))):
                        self.ReqMajor = int(Z.group('Major'))
                        self.ReqMinor = int(Z.group('Minor'))
                        self.moreRecentExists = True
                        logger.debug('found a more recent version: %02d.%02d' % (
                            int(Q['Major']), int(Q['Minor'])))
            return ret
        else:
            return ret
            
        
    def Download(self, Major=None, Minor=None, Cache=None):
        if not (Major or Minor):
        ...
        if Version_Info and cCache:
            for XT in ['deb', 'sum', 'tar.bz2']:
                ...
                SO, SE = utils.popen_wrapper(['/usr/bin/rsync',
                                              '--partial',
                                              '--times',
    ...
    def ClearsignedBy(self, Cache=None, Version=(0,0)):
        """
        ...
        if not Sum:
            return False
        SO, SE = utils.popen_wrapper(['sha512sum',
                                      '%s/%s' % (self.Cache,
    ...
    def __tar_unpack(self):
        Version_Info = self.GetVersion(Major=self.ReqMajor,
                                       Minor=self.ReqMinor)
        File_List, Errors = utils.popen_wrapper(
            ['/bin/tar',
             '--directory', '/',
             '-xvf',
             '%s/%s' % (self.Cache,
    ...
    def Cleanup(self):
        ...
        utils.popen_wrapper(['/bin/rm', '-rf', self.Cache], sudo=False, debug=False)
